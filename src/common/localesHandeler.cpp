...
{
    if(!std::filesystem::is_directory(app::common::global::APPDATA + "\\settings.ini"))
    {
    ...
    if(!std::filesystem::is_directory("locales\\" + g_appLanguage))
    {
        app::common::log::LogToFile("application", "[LOCALISATION_MANAGER] No .local file detected, delete settings.ini and restart application");
